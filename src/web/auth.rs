...
use actix_web::{cookie::{Cookie, SameSite}, dev::ServiceResponse, error::ErrorInternalServerError, get, middleware::ErrorHandlerResponse, web::{self, Data}, FromRequest, HttpRequest, HttpResponse, Responder};
use log::error;
use reqwest::header::LOCATION;
use serde::Deserialize;
use std::future::Future;
use oauth2::{basic::BasicClient, http::HeaderValue, reqwest::async_http_client, AuthUrl, AuthorizationCode, ClientId, ClientSecret, CsrfToken, RedirectUrl, TokenResponse, TokenUrl};
...
impl User {
    ...
    async fn from_token(access_token: &str, oauth_client : &OauthClient) -> Result<Self, actix_web::Error> {
        ...
        Ok(User {
            username: userinfo.get("preferred_username").map(|a| a.to_string()).ok_or(ErrorInternalServerError("Internal Error"))?,
            userinfo
        })
    }
}
...
pub(crate) fn oauth_client() -> OauthClient {
    ...
    OauthClient {
        client: BasicClient::new(ClientId::new(client_id),
            ...
            AuthUrl::new("https://login.inphima.de/auth/realms/FSCS-Intern/protocol/openid-connect/auth".to_string()).unwrap(),                Some(TokenUrl::new("https://login.inphima.de/auth/realms/FSCS-Intern/protocol/openid-connect/token".to_string()).unwrap())),
...
fn redirect_url<'a>(path: &str, request: HttpRequest) -> Cow<'a, RedirectUrl>{
    ...
    std::borrow::Cow::Owned(
        RedirectUrl::new(format!("{}://{}/auth/callback/?path={}", scheme, host, path)).unwrap())
}

#[get("/login/")]
async fn login(oauth_client: web::Data<OauthClient>, path: web::Query<PathParam>, request: HttpRequest) -> impl Responder {
   let path = path.into_inner().path.unwrap_or("/".to_string()); 

       let (ref mut auth_url, csrf_token) = &mut oauth_client.client
    ...
    HttpResponse::Found()
        ...
        .cookie(Cookie::build("csrf", csrf_token.secret()).same_site(SameSite::None).path("/auth/").finish())
        .finish()
}
...
async fn callback(
    ...
    HttpResponse::Found()
        ...
        .cookie(actix_web::cookie::Cookie::build("access_token", access_token).expires(None).same_site(SameSite::Lax).path("/").finish())
        .finish()
}
...
pub fn not_authorized<B>(
    ...
    *res.status_mut() = actix_web::http::StatusCode::FOUND;
    ...
    res.headers_mut().append(LOCATION, HeaderValue::from_str(&format!("/auth/login/?path={path}")).map_err(|_| ErrorInternalServerError("Invalid Path"))?);
