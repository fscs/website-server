...
impl<T: Sync> TimedCache<T> {
    ...
    pub(crate) fn with_generator<FN: Fn() -> Pin<Box<dyn Future<Output = T>>> + 'static + Sync>(generator: FN, duration: Duration) -> Self {
        TimedCache {
    ...
    pub(crate) async fn get(&self) -> impl Deref<Target = T> + '_ {
        ...
        if (&data).iter().any(|d| d.last_updated + self.duration < SystemTime::now()) {
            ReadWrapper(data)
        } else {
            drop(data);
            let mut write = self.data_last_updated.write().await;
            // Write only needed if not already updated
            if (*write).is_none() || (*write).iter().any(|d| d.last_updated + self.duration < SystemTime::now()) {
